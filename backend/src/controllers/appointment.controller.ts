...
export async function reschedule(req: Request, res: Response) { const auth = actor(req); res.json({ success: true, data: await appointments.updateAppointment(auth.businessId, auth.id, id(req), req.body, { reschedule: true }) }); }
