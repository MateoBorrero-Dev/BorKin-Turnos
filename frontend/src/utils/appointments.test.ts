...
import { agendaPath, appointmentError, availabilityTimes, calendarRange, clearAfterEmployeeOrDateChange, clearAfterServiceChange, clientOptionsPath, intervalIntersectsLocalDay, navigateAnchor, statusActions, todayKeyInZone } from "./appointments";
...
describe("agenda y formulario de turnos", () => {
  ...
  it("habilita botones sólo para transiciones válidas", () => { expect(statusActions("PENDIENTE")).toEqual(["confirm", "start", "cancel", "no-show"]); expect(statusActions("EN_CURSO")).toEqual(["complete"]); expect(statusActions("COMPLETADO")).toEqual([]); });
