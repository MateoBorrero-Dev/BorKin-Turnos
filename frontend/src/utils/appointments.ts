...
export function statusActions(status: AppointmentStatus) { return status === "PENDIENTE" ? ["confirm", "start", "cancel", "no-show"] : status === "CONFIRMADO" ? ["start", "cancel", "no-show"] : status === "EN_CURSO" ? ["complete"] : []; }
